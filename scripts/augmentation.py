...
class Augmentation:
    ...
    def point_cloud_callback(self, data):
        ...
        interpolation_points = self.interpolate_range(signal_loss_range)
        data = self.augment_points(data, interpolation_points + bump_points)
        
        # Publish results
        self.point_cloud_publisher.publish(data)
        self.signal_loss_range_publisher.publish(
            self.create_range_marker(signal_loss_range, data.header))
        
    def get_points(self, data, target_ring=[], ground_min=-0.05, ground_max=0.2, i_min=30):
    ...
    def search_signal_loss_range(self, points, dphi_min=0.05, d_max=1.0, i_accept=50):
        signal_loss_range = []
        for ring in points.keys():
    ...
    def augment_points(self, data, points):
        for p in points:
